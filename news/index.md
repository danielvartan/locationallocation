...
- Added [`lintr`](https://lintr.r-lib.org/) for code consistency. The
  package now follow the [tidy tools
...
- Added the demo data as exports, and changed their names to:
...
- Added `CITATION.cff` and `codemeta.json` files for better citation and
  metadata.
- Removed the `fasterize` dependency due to compatibility issues on
  Linux systems.
